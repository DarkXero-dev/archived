...
This is where you will be able to select what packages you want to install. We have spent a long time curating a list of interesting Tools for you to enjoy. (Will be constantly updated). Also this does not provide "Everything", it's just a list of curated useful apps & tools, for more either use Terminal or GUI package manager of choice. 

However, this first page mostly includes packages either from our Repos or ArchLinux ones. Since we aim at stability, we opted to offer the important stuff as Flatpaks, so hit the  button below labeled as such to switch, and find them there. If you dislike Flatpaks, you are free to install packages from whatever other sources you want, it's all up to you. Have fun !
"#,
        );
